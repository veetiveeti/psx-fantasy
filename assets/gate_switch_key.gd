...
var is_locked = true  # New variable to track locked state
...
func _ready():
	add_to_group("locked_switch")  # Add to group for key detection

func _process(_delta):
	if player_in_range:
		if Input.is_action_just_pressed("interact") and not is_locked:  # Only allow interaction if unlocked
			if is_gate_open:
				close_gate()
			else:
				open_gate()
...
func _on_body_entered(body):
	if body.is_in_group("player"):
		...
		if is_locked:
			show_interaction_text.emit("This switch requires a key.")
		else:
